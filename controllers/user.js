...
export const register = async(req,res)=>{
  ...
  ///after uploading deleting the local temp folder
  fs.rmSync("./tmp", { recursive: true });
    ...
    sendToken(user , res ,"OTP sent to your email, please Verify it", 201)
    }catch(error){
...
export const verify= async(req,res)=>{
    try{
        ...
        sendToken(user,res,"Account Successfully Verified",200);
...
export const login = async (req, res) => {
    try {
      ...
      if (!user.verified) {
        return res
          .status(400)
          .json({ success: false, message: "Please Verify Your Email within 5 minutes " });
      }
